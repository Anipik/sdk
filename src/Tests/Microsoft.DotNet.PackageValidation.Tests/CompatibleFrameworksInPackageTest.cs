...
{
    public class CompatibleFrameworksInPackageTest  : SdkTest
    {
        public TestLogger _logger;

        public CompatibleFrameworksInPackageTest(ITestOutputHelper log) : base(log)
        {
            _logger = new();
        }
...
namespace PackageValidationTests
{
    public class First
    {
        ...
        public void test(bool test) { }
            ...
            // TODO: add asserts for assembly and header metadata.
            Assert.Contains("CP0002 : Member 'PackageValidationTests.First.test(string)' exists on the left but not on the right", _logger.errors);
            Assert.Contains("CP0002 : Member 'PackageValidationTests.First.test(bool)' exists on the left but not on the right", _logger.errors);
        }
    }
}
